...
const mapclass <- class mapclass
  ...
  for i: integer <-0 while i< xdim by i <- i+1
    private_map[i] <- vec.create[ydim] %vec is just a vector type, so call like this to create each time
    end for
  end init
...
const generator <- class genclass
  ...
  attached var it:Integer <- 0 %denne oeker load
  ...
  process % happens after .create becaus its in a class
    loop
      exit when !trap % wait for setWindow to complete
    end loop
...
% så dette fungerte med en passive ventdene node og en active (den man starter med)
const runner <- object runner
  initially
    stdout.putString["running...\n"] stdout.flush
    % params 400, 200, 100 fungerer
    master.params[800,400,300] % param0 bestemmer hvor stor x aksen er på bildet. param1 er y, siste er kvaliteten på bildet
    stdout.PutString["params done...\n"] stdout.flush
    master.setwindow[-2.5,1.0,-1.0,1.0]
    stdout.putstring["setwindow done...\n"] stdout.flush
    master.mandelbroot[2]
